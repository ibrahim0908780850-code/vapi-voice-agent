...
router.post("/create", authenticateRequest, requireTenantIdentity, rejectTenantMismatch, async (req, res) => {
  try {
    ...
    const { error: contentError } = await supabase.from("website_content").insert({
      tenant_id,
      hero_title: "Welcome To Your Website",
      hero_description: "Your AI powered business website",
      services: {}, faq: {}, contact_info: {}
    });
    ...
    const { error: pagesError } = await supabase.from("website_pages").insert([
      { tenant_id, page_name: "Home", slug: "/", content: {} },
      { tenant_id, page_name: "About", slug: "about", content: {} },
      { tenant_id, page_name: "Services", slug: "services", content: {} },
      { tenant_id, page_name: "Contact", slug: "contact", content: {} }
    ]);
...
router.get("/:tenant_id", authenticateRequest, requireTenantIdentity, async (req, res) => {
  try {
    ...
    const { data, error } = await getSupabase().from("websites")
...
async function updateWebsite(req, res, changes) {
  try {
    const tenant_id = req.tenantId;
    const supabase = getSupabase();
    if (!(await websiteBelongsToTenant(supabase, req.params.id, tenant_id))) {
