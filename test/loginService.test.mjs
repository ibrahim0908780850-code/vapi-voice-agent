    ...
    from(table) {
      const query = {
        select(columns) {
          return {
            eq() {
              if (table === "company_requests") return { order() { return { limit() { return { async maybeSingle() { return { data: null, error: null }; } }; } }; } };
              return { async maybeSingle() { return { data: columns === "id, auth_user_id" ? { id: "profile-1", auth_user_id: "auth-1" } : table === "users" ? user : table === "tenants" ? { status: "active" } : null, error: null }; } };
            }
          };
        }
      };
